...
<head>
    ...
    <title>Crear Sitio Turístico</title>
    <style>
        body {
...
<body>
    <x-app-layout>
        <x-slot name="header">
            <h2 class="title">
                {{ __('Agregar un nuevo sitio turístico') }}
            </h2>
        </x-slot>

        <div class="container">
            <div class="form-container">
                <form method="POST" action="{{ route('lugar.store') }}" id="create-form">
                    @csrf
                    <div class="mb-3">
                        <label for="name" class="form-label">Nombre del sitio turístico:</label>
        ...
        document.getElementById('create-form').addEventListener('submit', function(event) {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
                        title: data.message || 'Sitio turístico creado correctamente',
                        showConfirmButton: false,
                        timer: 2000
                    }).then(() => {
                        window.location.href = "{{ route('lugar.index') }}";
                    });
                } else {
                    Swal.fire({
                        position: 'center',
                        icon: 'error',
                        title: 'Error',
                        text: data.error || 'Error al crear el sitio turístico',
            ...
            .catch(error => {
                ...
                Swal.fire({
                    position: 'center',
                    icon: 'error',
                    title: 'Error al crear el sitio turístico',
